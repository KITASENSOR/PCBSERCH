...
2026-07-14 Excel覆蓋D1與固定料顯示修正-Business
...
2026-07-15機台連續時間記錄預覽-Business

    - 「目標數量算時間」新增連續時間記錄，可將目前試算結果保存為一組設定，並自動把預估完成時間帶入下一組起始時間。
    - 新增換線時間項目，只需輸入分鐘數，不必填寫生產數量與單件工時；換線結束時間會接續成下一組起始時間。
    - 時間記錄位於試算頁最下方，每筆只顯示開始與結束時間；點擊後可把該筆設定帶回上方輸入欄位。
    - 時間記錄支援跨日標示、逐筆刪除、整批清除及瀏覽器本機保存。
    - 本次當時先供 Codex 瀏覽器本機 HTML 預覽，後續正式部署資訊請見 V6 紀錄。


2026-07-15機台時間記錄手機版面修正-Business

    - 移除連續記錄與記錄清單的說明文字。
    - 生產記錄按鈕縮短為「＋」，並與「帶入現在時間」、「清除」固定在同一列。
    - 修正手機版時間記錄標題受到清除按鈕擠壓而呈現直排的版面問題。


2026-07-15時間記錄覆寫與連動重算-Business

    - 點擊生產記錄後可修改上方欄位，原「＋」按鈕會切換為「儲存」，按下後覆寫原記錄。
    - 點擊換線記錄後可修改開始時間與換線分鐘數，「加入換線時間」會切換為「儲存修改」。
    - 修改任一筆記錄後，後續記錄會依新的結束時間重新排列，並同步更新瀏覽器本機保存內容。


2026-07-15換線記錄獨立新增-Business

    - 「加入換線時間」改為只新增換線項目，不再連帶新增或覆寫上方尚未保存的生產設定。
    - 生產設定僅能透過「＋」或編輯狀態下的「儲存」寫入時間記錄。


2026-07-15新增記錄自動接續時間-Business

    - 已有時間記錄時，新增生產設定與換線時間都固定以前一筆記錄的結束時間作為開始時間。
    - 第一筆記錄仍使用上方輸入的目前時間；編輯既有記錄時保留該筆開始時間供修改。


2026-07-15機台連續時間記錄正式部署-Business

    - 功能提交 `bc227f2968bfd5c1ea4edb6b4b623b098b31bf90` 已推送至 GitHub `main`。
    - GitHub Actions 執行編號 `29395130878` 已成功完成 Cloudflare 部署。
    - Cloudflare 正式版本 ID：`0353e4bf-f070-4d35-9051-ca7c29ad3f27`。
    - 正式網址 `https://smt-pcb-search.s110513202.workers.dev/` 已驗證為 HTTP 200，並確認包含連續時間記錄、記錄覆寫、換線記錄及自動接續前一筆結束時間功能。
    - 本次只有 Worker Assets 前端與文件變更，沒有 D1 schema 或資料異動。


2026-07-15時間記錄組別名稱編輯-Business

    - 生產時間記錄的預設名稱維持「第 1 組設定」、「第 2 組設定」。
    - 點擊組別名稱文字會切換成輸入框，可按 Enter 或點擊其他位置保存自訂名稱，按 Esc 取消。
    - 自訂名稱會寫入瀏覽器本機保存內容；修改該筆計算值時仍會保留自訂名稱。
    - 換線項目維持固定名稱，不提供重新命名。


2026-07-15時間記錄組別名稱正式部署-Business

    - 功能提交 `ea5888048bd64749d97338dda28ae158cc9a56b0` 已推送至 GitHub `main`。
    - GitHub Actions 執行編號 `29395585639` 已成功完成 Cloudflare 部署。
    - Cloudflare 正式版本 ID：`c68a2ea3-764c-4068-bb33-52adee56e87c`。
    - 正式網址已驗證為 HTTP 200，並確認包含組別名稱編輯、自訂名稱保存及修改計算值後保留名稱功能。
    - 本次沒有 D1 schema 或資料異動。


2026-07-15時間記錄線別切換-Business

    - 時間記錄標題列新增固定「1線」、「2線」切換按鈕，位置位於標題與「清除記錄」之間。
    - 上方輸入欄位、計算模式、試算結果、換線分鐘與下方時間記錄都以目前線別為準，各線分開保存。
    - 現有「＋」與「加入換線時間」直接寫入目前線別，不增加選擇加入線別的按鈕。
    - 「清除」與「清除記錄」只影響目前線別，不會清除另一條線。
    - 新版首次載入會直接移除舊版單線時間資料，1線與2線皆從空白開始，不建立預設記錄。


2026-07-15時間記錄線別切換正式部署-Business

    - 功能提交 `82f82fc8739bcdb014f8b8d7d6a4cb3107b8dfc8` 已推送至 GitHub `main`。
    - GitHub Actions 執行編號 `29396284484` 已成功完成 Cloudflare 部署。
    - Cloudflare 正式版本 ID：`97f6f08c-cd10-4fd7-b803-fd89ae1472f8`。
    - 正式網址已驗證為 HTTP 200，並確認包含1線／2線切換、分線輸入、分線記錄、目前線別清除及舊資料清除功能。
    - 本次沒有 D1 schema 或資料異動。


2026-07-15機台時間預設模式與記錄顯示-Business

    - 機台時間頁預設切換為「目標數量算時間」，兩個線別重新載入後都以此模式開啟。
    - 連續時間記錄、線別切換、換線功能與下方時間記錄只在「目標數量算時間」顯示。
    - 「剩餘時間算數量」維持原有輸入、計算結果、「帶入現在時間」與「清除」，不顯示時間記錄相關介面。


2026-07-15機台時間預設模式正式部署-Business

    - 功能提交 `4d80b5758ad1a17a304da9e7dd9149e652efaf8f` 已推送至 GitHub `main`。
    - GitHub Actions 執行編號 `29396911979` 已成功完成 Cloudflare 部署。
    - Cloudflare 正式版本 ID：`5344953b-a606-47ca-b293-0fc7b3dc835d`。
    - 正式網址已驗證為 HTTP 200，預設模式為「目標數量算時間」，且時間記錄相關介面只在該模式顯示。
    - 本次沒有 D1 schema 或資料異動。


2026-07-17取料頁面新增下料帶入-Business

    - 取料頁面在「查BOM」旁新增「下料」按鈕，可將目前選取的型號貼入下料頁面。
    - 下料頁面已有內容時，可選擇清空後貼上或附加在原內容後方，並同步保存於瀏覽器本機。
    - 「讀排程」、「倉管」、「查BOM」、「下料」、「清除」五個按鈕固定在同一排顯示。
    - 本次只有 Worker Assets 前端與文件變更，沒有 D1 schema 或資料異動。


2026-07-20新增A／B區塊共用料比對頁-Business

    - 主選單新增「比對」頁，固定放在「下料」與「取料」之間，五個入口維持同一列。
    - 比對頁的 A 區塊輸入單一完整型號，B 區塊可輸入多個型號或系列關鍵字。
    - 結果只列出 A 型號與 B 區塊至少一個型號共同使用的物料，並逐料標示實際共用的 B 型號。
    - 比對輸入會保存於瀏覽器本機，固定料會在結果中加上標示。
    - 本次只有 Worker Assets 前端與文件變更，沒有 D1 schema 或資料異動。


2026-07-20比對結果與A區塊合併規則修正-Business

    - 比對結果改為與查BOM相同的「類別／子件明細」表格，不再顯示各料對應的 B 型號。
    - 成功狀態只顯示「找到 N 種共用料」，不再顯示 A、B 型號摘要或其他比對細節。
    - A 區塊改用查BOM既有的型號合併規則；例如輸入 `42604A` 時，10 個相同 BOM 的顏色尾碼會合併視為 `PC-0426-04A-XX` 單一型號。
    - 本次只有 Worker Assets 前端與文件變更，沒有 D1 schema 或資料異動。


2026-07-20取料勾選流程與比對帶入調整-Business

    - 「讀排程」改為重新整理圖示，移至排程列表收闔鍵旁，兩個圖示按鈕尺寸相同。
    - 移除長按選取功能；排程勾選後只加入選取清單，不立即計算物料。
    - 有勾選排程時，排程列表標題旁才顯示「計算」與「完工」按鈕，兩者共用目前勾選項目。
    - 取料頁上方在「下料」與「清除」之間新增「比對」，可將勾選型號貼入比對頁 B 區塊。
    - 本次只有 Worker Assets 前端與文件變更，沒有 D1 schema 或資料異動。
